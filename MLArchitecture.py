...
#sel_feat[i] = librosa.util.normalize(train_mfccs.reshape(-1, train_mfccs.shape[-1])).reshape(train_mfccs.shape)
...
#Loading RMS energy stats ---> NEVER USED
...
#For Random Forest only ---> REMOVED FROM THE REPORT
...
"""with open('trainMelSpec_norm.npy', 'wb') as f:
    np.save(f, sel_feat)

with open('testMelSpec5_norm.npy', 'wb') as f:
    np.save(f, sel_test5)

with open('testMelSpec7_norm.npy', 'wb') as f:
    np.save(f, sel_test7)

with open('testMelSpec8_norm.npy', 'wb') as f:
    np.save(f, sel_test8)

with open('testMelSpec9_norm.npy', 'wb') as f:
    np.save(f, sel_test9)

with open('MelSpec10_norm.npy', 'wb') as f:
    np.save(f, sel_test10)"""
...
#dataset = tf.data.Dataset.from_tensor_slices(chroma_feat, train_labels)
...
#First model, RF --> REMOVED FROM THE REPORT
...
denseModel = tf.keras.Sequential([
    layers.Flatten(input_shape=(params['dim'][1], params['n_channels'])),
